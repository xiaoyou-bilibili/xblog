...
// import { getNavigationLink } from '@/api/admin/plugins'
...
export default {
    ...
    return {
      ...
      tableProxy: {
        ajax: {
          ...
          // query: ({ page, form }) => getNavigationLink(navigationID, { page: page.currentPage, page_size: page.pageSize, search_type: form.search_type, search_key: form.search_key }),
          // 删除方法被触发
  ...
  // 把getter混入到computed对象中,我们可以直接调用vex里面的内容即可-->
    ...mapGetters('admin-plugins', ['navigation'])
  },
  mounted () {
  ...
  methods: {
    ...
    updateNavigation (update = false, show = true) {
      ...
      this.$store.dispatch('admin-plugins/getNavigations')
    ...
    saveRowEvent (row) {
      ...
      this.$refs.grid.clearActived().then(() => {
        ...
        if (isNaN(row.id)) {
          this.$store.dispatch('admin-plugins/addNavigation', data).then(_ => this.updateData()).catch((msg) => { this.$message.error(msg); this.updateData(false) })
        } else {
          this.$store.dispatch('admin-plugins/updateNavigation', { id: row.id, data }).then(_ => this.updateData()).catch(msg => this.$message.error(msg))
        }
      })
    },
    // 提示更新成功并刷新数据
    ...
    deleteDonate (id) {
      if (id === '') {
        ...
        // 删除赞助
        this.$store.dispatch('admin-plugins/deleteNavigation', id).then(_ => this.updateData()).catch(msg => this.$message.error(msg))
      }
    },
    // 获取导航下的网址
    getItems (id) {
    ...
    addNavigationCategory () {
      ...
      if (this.addOption) {
        this.$store.dispatch('admin-plugins/addNavigation', category).then(() => { this.updateNavigation(true) }).catch(msg => this.$message.error(msg))
      } else {
        this.$store.dispatch('admin-plugins/updateNavigation', { id: navigationID, data: category }).then(() => { this.updateNavigation(false) }).catch(msg => this.$message.error(msg))
      }
    },
    // 检查分类
    checkCategory () {
    ...
    deleteNavigation () {
      ...
      if (this.checkCategory()) { return }
      // 删除分类
      this.$store.dispatch('admin-plugins/deleteNavigation', navigationID).then(() => { this.updateNavigation(true) }).catch(msg => this.$message.error(msg))
    },
    // 添加网址
    addLink () {
      // 检查是否选择分类
