...
export default {
  ...
  computed: {
    ...
    ...mapGetters('admin-posts', ['categoryList'])
  },
  watch: {
  ...
  mounted () {
    this.$store.dispatch('admin-posts/getCategoryList')
  },
  methods: {
    ...
    addNewChapter (parent, choose = null) { // 新建章节信息
      this.$prompt('请输入分类名', '提示', {
        ...
        this.$store.dispatch('admin-posts/addCategory', { parent, name: value, type: 'category' }).then((data) => {
          // 自动向节点添加数据
          const newChild = { id: data.item_id, label: value, children: [] }
          // 判断是否是右键选中的对象
          if (choose !== null) {
            // 判断当前选中的节点是否有子节点
            if (!choose.children) {
            ...
            // 没有传值说明是根节点插入值
            this.$store.commit('admin-posts/pushCategoryList', newChild)
          }
        }).catch((msg) => { this.$message.error(msg) })
      }).catch(() => {})
    },
    updateChapter (data) { // 更新章节信息
      return this.$store.dispatch('admin-posts/updateCategoryList', data)
    },
    delChapter (data, node) { // 删除某个章节
      this.$confirm('删除该分类(该分类下所有子分类都会消失)?', '提示', {
        ...
        this.$store.dispatch('admin-posts/deleteCategory', data.id).then(() => {
          // 我们手动从文档列表里删除这个文档
          // 获取当前文档的父节点node信息
    ...
    changeChapterTitle (data) { // 修改章节标题
      this.$prompt('新标题', '提示', {
        ...
        this.updateChapter({ id: data.id, data: { name: value } }).then(() => {
          data.label = value
        }).catch((msg) => { this.$message.error(msg) })
      }).catch(() => {})
    },
