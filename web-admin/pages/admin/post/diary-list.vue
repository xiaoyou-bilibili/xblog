...
import { getDiary } from '@/api/admin/posts'
export default {
  layout: 'admin',
    ...
    return {
      ...
      tableProxy: {
        ajax: {
          ...
          query: ({ page, form }) => getDiary({ page: page.currentPage, page_size: page.pageSize, search_type: form.search_type, search_key: form.search_key }),
          // 删除方法被触发
      ...
      tableForm: {
        items: [
          { field: 'search_type', title: '搜索类型', itemRender: { name: '$select', options: [{ label: '日记id', value: 'post_id' }, { label: '日记内容', value: 'content' }] } },
  ...
  methods: {
    ...
    // 跳转到文章编辑器
    gotoEdit (id) {
      let path = '/admin/post/diary-edit'
      if (id !== 0) {
        path += '/' + id
      }
      // 跳转路由
      ...
      // 添加标签-->
      this.$store.dispatch('admin/editChangeId', { title: '日记编辑器', path })
    }
  }
}
</script>
